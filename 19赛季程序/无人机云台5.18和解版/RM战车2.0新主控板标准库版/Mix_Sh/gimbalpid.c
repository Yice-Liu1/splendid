...
float Kp_pi_po=300;    //cml:pitch轴 角度环PID 参数//180 -5 60 50 10 10
float Ki_pi_po=1.6;
float Kd_pi_po=50;
float out_pi_po=0;
...
//error[0] = 145 - angle_use;                            //cml:先以0°作为期望
  ...
  if(accumu_i_posi>1000) accumu_i_posi=1000;
	...
	out_pi_po =   Kp_pi_po * error[0] + Ki_pi_po * accumu_i_posi + Ki_pi_po * error[1];
							
/*****************************************************************************************************/

	
	
  error_speed[0] = 	out_pi_po - speed_pitch*3.5f;
...
//error_speed[1] = 加速度
	...
	out_pi_sp =   Kp_pi_sp * error_speed[0] + Kp_pi_sp * accumu_i_speed + Kp_pi_sp * error_speed[1];
...
/*************************************************/
 ...
 angle_speed_old = speed_pitch;
	...
	if(out_pi_sp>25000) out_pi_sp=25000;
	if(out_pi_sp<-25000) out_pi_sp=-25000;
...
/***********************************yaw_pid*****************************************/
float Kp_ya_po=100;      //cml:pitch轴 角度环PID 参数
float Ki_ya_po=0.01;
float Kd_ya_po=40;
float out_ya_po=0.0;
