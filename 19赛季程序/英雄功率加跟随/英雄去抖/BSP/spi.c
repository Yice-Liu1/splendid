...
{	 
  ...
  GPIO_Init(GPIOF, &GPIO_InitStructure);//初始化
	...
	//这里只针对SPI口初始化
//	RCC_APB2PeriphResetCmd(RCC_APB2Periph_SPI5,ENABLE);					//复位SPI1
//	RCC_APB2PeriphResetCmd(RCC_APB2Periph_SPI5,DISABLE);				//停止复位SPI1
...
void SPI5_SetSpeed(u8 SPI_BaudRatePrescaler)
{
  assert_param(IS_SPI_BAUDRATE_PRESCALER(SPI_BaudRatePrescaler));//判断有效性
	...
	SPI5->CR1|=SPI_BaudRatePrescaler;	//设置SPI1速度 
	SPI_Cmd(SPI5,ENABLE); //使能SPI1
} 
//SPI1 读写一个字节
//TxData:要写入的字节
//返回值:读取到的字节
