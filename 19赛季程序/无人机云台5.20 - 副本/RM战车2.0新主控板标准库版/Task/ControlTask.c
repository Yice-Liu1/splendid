...
int f,l,p,g,s1,s2,bl,bolun;//遥控通道
int F=0,L=0,P=0,S1;
float G=242;//pitch轴遥控角度变量
float Y=180;//YAW轴遥控角度变量
...
void remote(void)
{
    ...
    pic_pwm=1712;		//111
		//yaw_wish=20;//tmp
		yaw_wish+=((float)(p)/5000);
	  if(yaw_wish>=280) yaw_wish=280;
		if(yaw_wish<=10) yaw_wish=10; 
    start2=1;		
	}
	else if(s1==2)//下
	{	
		if(start1) PWM_Picture();			
		start1=0;
		pic_pwm=1760;
		pitch_wish+=((float)(f)/5000); 	
   	if(pitch_wish>=25)   pitch_wish=25;
		if(pitch_wish<=-30) pitch_wish=-30;  //cml:限制G的范围
	...
	else if(s1==1)//上
	{
      ...
      pic_pwm=2000;	}
		start2=0;
		pic_pwm+=((float)(bolun)/3000);//111
		if(pic_pwm>=1960)  pic_pwm=1960;
		if(pic_pwm<=1712)  pic_pwm=1712;
    pitch_wish+=((float)(f)/5000);
   	if(pitch_wish>=25)   pitch_wish=25;
		if(pitch_wish<=-20) pitch_wish=-20;  
	...
	{ 
	  SetFrictionWheelSpeed(1150);//111
	}

		 
	if(l!=0&&g==0)//一两发
	{
		FRICTIONPID(1000);//111
	}
	
	if(g>0)//20发
	{
	  FRICTIONPID(3500);//111
	}
		
	if(g<0)//10发
	{ 
		FRICTIONPID(2000);//111
	}
...
//	if(l<0||g!=0)
//	{
//		SetFrictionWheelSpeed(1150);
//		FRICTIONPID(2100);	
////		if(p>0) FRICTIONPID(-2100);
////		if(p<0) FRICTIONPID(2100);	
//	}
...
void GimbalPID(void)
{
	...
	if(G<=1000)
		G=1000;
	//云台控制输出								
	GMPPositionPID.ref=2188;
	GMPPositionPID.fdb=Yp; 
	GMYPositionPID.ref=G;
...
Set_Current(CAN1, -Yaw_End_Out, -Pitch_End_Out,ShootMotorSpeedPID.output);
