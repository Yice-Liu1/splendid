...
void USART3_IRQHandler(void)
{
	...
	{
		u8 com_data = USART3->DR;//USART_ClearFlag(USART3,USART_IT_ORE);
	}
	//发送中断
	if((USART3->SR & (1<<7))&&(USART3->CR1 & USART_CR1_TXEIE))//if(USART_GetITStatus(USART1,USART_IT_TXE)!=RESET)
	{
		USART3->DR = TxBuffer[TxCounter++]; //写DR清除中断标志          
		if(TxCounter == count)
		{
			USART3->CR1 &= ~USART_CR1_TXEIE;		//关闭TXE中断//USART_ITConfig(USART1,USART_IT_TXE,DISABLE);
		}
	}
	//接收中断 (接收寄存器非空) /////////////////////////////////////////////////////////////////////////////////////////
	if(USART3->SR & (1<<5))  
