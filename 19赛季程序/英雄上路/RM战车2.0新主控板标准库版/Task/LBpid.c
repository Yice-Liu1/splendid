...
//内环PID参数
float Pitch_core_kp=0;      //cml:之前版本是15   6
float Pitch_core_ki=0;
float Pitch_core_kd=0;     //cml:之前版本5   5

float Pitch_core_i=0.0;
...
////////////////////////内环角速度环(PD)///////////////////////////////   
	...
	//Pitch_core_i = Pitch_core_i + ();
	
 // pitch_core_kp_out = Pitch_core_kp * (Pitch_shell_out -  GYRO_Pitch );	//cml:改。
 pitch_core_kp_out = Pitch_core_kp * (Pitch_shell_out +  GYRO_Pitch * 2.5f);
...
//外环PID参数
float Yaw_shell_kp=700;//
float Yaw_shell_ki=0.1;//
float Yaw_shell_kd=800;//
float Speed_Yaw;		//速度保留
float Yaw_core_kp_out,Yaw_core_kd_out;	//内环单项输出
float Yaw_core_ki_out=0.0;
//内环PID参数5
float Yaw_core_kp=150;
float Yaw_core_ki=0.1;
float Yaw_core_kd=270;

float core_jifen=0.0;
...
int Yaw_PID(float RC_Yaw,float GYRO_Yaw,float Encoder_Angle)
{
  ...
  Yaw_i+=(Encoder_Angle-RC_Yaw);       //积分的累加
  if(Yaw_i>1000) Yaw_i=1000;           //积分和的限制
  else if(Yaw_i<-1000) Yaw_i=-1000;
  ...
  Yaw_old=Encoder_Angle;                //角度保存,供微分使用



//	core_jifen = core_jifen + (//chazhi)

  Yaw_core_kp_out = Yaw_core_kp * (Yaw_shell_out +  GYRO_Yaw * 5.0f);
  //Yaw_core_ki_out = Yaw_core_ki * core_jifen;
  Yaw_core_kd_out = Yaw_core_kd * (GYRO_Yaw - Speed_Yaw_Old);
...
void Gimbal_Set(float RC_Pitch,float RC_Yaw,float GYRO_Pitch,float GYRO_Yaw,float Pitch_Encoder_Angle,float Yaw_Encoder_Angle)
{
  ...
  //Yaw_Fina_Out=0;//cml:为尝试单个赋值
	Set_Gimbal_Current(CAN1,0,Pitch_Fina_Out);
	//delay_us(0);
