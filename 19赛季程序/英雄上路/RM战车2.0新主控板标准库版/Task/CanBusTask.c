...
cxg摩擦轮输出函数
**************************************/
void Set_Current(CAN_TypeDef *CANx,int16_t gimbal_yaw_iq, int16_t gimbal_pitch_iq, int16_t friction_iq ,int16_t fighting)
{
    CanTxMsg tx_message;    
    tx_message.StdId = 0x1FF;
    tx_message.IDE = CAN_Id_Standard;
...
//    tx_message.Data[3] = (unsigned char) gimbal_pitch_iq;
    ...
    tx_message.Data[6] = (unsigned char)(fighting >> 8);
    tx_message.Data[7] = (unsigned char)fighting;
    CAN_Transmit(CANx,&tx_message);
}
