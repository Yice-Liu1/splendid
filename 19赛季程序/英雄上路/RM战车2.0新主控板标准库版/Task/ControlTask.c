...
PID_Regulator_t SmallShootSpeedPID = SHOOT_MOTOR_SPEED_PID_DEFAULT;
PID_Regulator_t BigShootSpeedPID = CHASSIS_MOTOR_SPEED_PID_DEFAULT;
...
//	G+=((float)(-g)/800); 				//cml: G的值和遥控通道的关系？  G的初始值为265 
	...
	if(l==-1024) stop();            //cml:未接时RC_CtrlData.rc.chx默认为0.
	else if(f!=0||l!=0||p!=0||g!=0)
	{
...
////	if(s2==3)
////	{
////		SetFrictionWheelSpeed(speed_number);
...
void FRICTIONPID(int f)
{
	...
	Set_Current(CAN1, 0, 0,ShootMotorSpeedPID.output,1000);
...
void Big_Small_Bang(int BIG,int SMALL)
{
	BigShootSpeedPID.ref = BIG;
	BigShootSpeedPID.fdb = M8;
	PID_Calc(&BigShootSpeedPID);
	if(BigShootSpeedPID.output>=4500) BigShootSpeedPID.output=4500;
	if(BigShootSpeedPID.output<=-4500) BigShootSpeedPID.output=-4500;
	
	SmallShootSpeedPID.ref=SMALL;
	SmallShootSpeedPID.fdb=M5;
	PID_Calc(&SmallShootSpeedPID);
	if(SmallShootSpeedPID.output>=4500) SmallShootSpeedPID.output=4500;
	if(SmallShootSpeedPID.output<=-4500) SmallShootSpeedPID.output=-4500;
	
	Set_Current(CAN1, 0, 0,SmallShootSpeedPID.output,1000);
}
