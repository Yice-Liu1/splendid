...
float Pitch_shell_kp=500;//      4hao:90 0.2 50     
float Pitch_shell_ki=0.2;//   250 0.2 300 12 200
float Pitch_shell_kd=500;//   800     500 6  500
float Speed_Pitch;		//速度保留
float pitch_core_kp_out,pitch_core_kd_out;	//内环单项输出
//内环PID参数
float Pitch_core_kp=4;       //4hao 15 5
float Pitch_core_kd=200;
...
////////////////////////内环角速度环(PD)///////////////////////////////       
  ...
  if(Pitch_core_out>=4500)		Pitch_core_out=4500;	
	if(Pitch_core_out<=-4500)	Pitch_core_out=-4500;
...
//外环PID参数
float Yaw_shell_kp=1000;//250 0.2 300 12 200
float Yaw_shell_ki=0.02;//
float Yaw_shell_kd=800;//
float Speed_Yaw;		//速度保留
float Yaw_core_kp_out,Yaw_core_kd_out;	//内环单项输出
//内环PID参数
float Yaw_core_kp=0.3;
float Yaw_core_kd=200;
...
////////////////////////内环角速度环(PD)///////////////////////////////       
  Yaw_core_kp_out = Yaw_core_kp * (Yaw_shell_out +  GYRO_Yaw * 4.5f);
...
//--------------------输出到电机--------------------------------//
	...
	if(Yaw_shell_out>=4500)		Yaw_shell_out=4500;	
	if(Yaw_shell_out<=-4500)	Yaw_shell_out=-4500;
