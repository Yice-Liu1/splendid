...
{int speed;
			...
			l=(RC_CtrlData.rc.ch0+RC_CtrlData.mouse.y-1024);//右边左右
			f=(RC_CtrlData.rc.ch3-1024);	//左边垂直
			p=(RC_CtrlData.rc.ch2-1024);//左边左右
			g=(RC_CtrlData.rc.ch1-1024);//右边垂直
			s1=RC_CtrlData.rc.s1;//决定是yaw还是pitch	
			s2=RC_CtrlData.rc.s2;//摩擦轮控制
			bolun=(RC_CtrlData.bolun+RC_CtrlData.mouse.x-1024);	
      ...
      SetFrictionWheelSpeed(0);			
		...
		if(RC_CtrlData.mouse.press_r||s1!=2) 
		{
			shoot=1000;
