...
void GetEncoderBias(volatile Encoder *v, CanRxMsg * msg)
{
            ...
            v->ecd_bias = (msg->Data[0]<<8)|msg->Data[1];  //保存初始编码器值作为偏差  //初始编码器机械角
            v->ecd_value = v->ecd_bias;      //保存初始编码器
            v->last_raw_value = v->ecd_bias;      //保存为上一次的                 
            v->temp_count++;
}
  ...
  void EncoderProcess(volatile Encoder *v, CanRxMsg * msg)
{
	int i=0;
	int32_t temp_sum = 0;    
	v->last_raw_value = v->raw_value;    //保存为上一次编码器的值
	v->raw_value = (msg->Data[0]<<8)|msg->Data[1];       //此时编码器值
	v->diff = v->raw_value - v->last_raw_value;       //两次差值
	...
	//计算得到连续的编码器输出值
	v->ecd_value = v->raw_value + v->round_cnt * 8192;//此时编码器值加圈数*8192
	//计算得到角度值，范围正负无穷大
	//v->ecd_angle = (float)(v->raw_value - v->ecd_bias)*360/8192 + v->round_cnt * 360;
	v->ecd_angle = ((float)(v->raw_value - v->ecd_bias)*360/8192 + v->round_cnt*360)/27;		
}
...
//	//计算得到角度值，范围正负无穷大
//	//v->ecd_angle = (float)(v->raw_value - v->ecd_bias)*360/8192 + v->round_cnt * 360;
//	v->ecd_angle = ((float)(v->raw_value - v->ecd_bias)*360/8192 + v->round_cnt*360)/19;
...
/*************************************
zh:两侧推杆(3508)+上岛小轮（2006）
**************************************/
void Arcoss_Vice(CAN_TypeDef *CANx, int16_t arcoss2wheel_iq,int16_t arcoss1wheel_iq, int16_t vice_fwheel_iq, int16_t vice_hwheel_iq)
{
	  CanTxMsg tx_message;    
    tx_message.StdId = 0x1FF;
    tx_message.IDE = CAN_Id_Standard;
    ...
    tx_message.Data[0] = (unsigned char)(arcoss2wheel_iq>>8);//5
	  tx_message.Data[1] = (unsigned char)arcoss2wheel_iq;
    tx_message.Data[2] = (unsigned char)(arcoss1wheel_iq>>8);//6
    tx_message.Data[3] = (unsigned char)arcoss1wheel_iq;
    tx_message.Data[4] = (unsigned char)(vice_fwheel_iq>>8);
    tx_message.Data[5] = (unsigned char)vice_fwheel_iq;
    tx_message.Data[6] = (unsigned char)(vice_hwheel_iq>>8);
    tx_message.Data[7] = (unsigned char)vice_hwheel_iq;
    CAN_Transmit(CANx,&tx_message);
}

void Set_GETarm(CAN_TypeDef *CANx, int16_t arm_iq)
{
    CanTxMsg tx_message;
    tx_message.StdId = 0x200;
    tx_message.IDE = CAN_Id_Standard;
    ...
    tx_message.Data[0] = (uint8_t)(arm_iq >> 8);
    tx_message.Data[1] = (uint8_t)arm_iq;
...
void Set_GEThand(CAN_TypeDef *CANx, int16_t hand_iq)
{
    CanTxMsg tx_message;
    tx_message.StdId = 0x200;
    tx_message.IDE = CAN_Id_Standard;
    ...
    tx_message.Data[2] = (uint8_t)(hand_iq >> 8);
    tx_message.Data[3] = (uint8_t)hand_iq;
    tx_message.Data[4] = 0x00;
    tx_message.Data[5] = 0x00;
    tx_message.Data[6] = 0x00;
    tx_message.Data[7] = 0x00;
...
//Set_CM_Speed(CAN1,600,600,600,600);
	//delay_ms(20);
	CM1SpeedPID.ref=(CM1SpeedPID.ref+speed);
	CM2SpeedPID.ref=(CM2SpeedPID.ref+speed);
	CM3SpeedPID.ref=(CM3SpeedPID.ref+speed);
	CM4SpeedPID.ref=(CM4SpeedPID.ref+speed);
}
/***************************************
底盘控制（顺时针）
****************************************/
...
//Set_CM_Speed(CAN1,-1500,1500,1500,-1500);
	...
	CM3SpeedPID.ref=(CM3SpeedPID.ref+speed);
	CM4SpeedPID.ref=(CM4SpeedPID.ref-speed);
	//vice_fwheelspeedPID.ref=-speed;
	//vice_bwheelspeedPID.ref=speed;
}
/***************************************
底盘控制(停止)
****************************************/
