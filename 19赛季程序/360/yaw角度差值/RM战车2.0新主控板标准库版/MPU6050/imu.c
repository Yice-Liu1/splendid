...
{  
    ...
    angles[0] = -atan2(2 * q[1] * q[2] + 2 * q[0] * q[3], -2 * q[2]*q[2] - 2 * q[3] * q[3] + 1)* 180/M_PI+en; // yaw        -pi----pi
	//  angles[0] = angles[0] + self_down;
	 // angles[0] = atan2(2 * q[1] * q[2] + 2 * q[0] * q[3], q[0]*q[0]+q[1]*q[1]-q[2]*q[2]-q[3]*q[3])* 180/M_PI;
...
void GetPitchYawGxGyGz()
{
	...
	last_yaw_temp = yaw_temp;                //cml:yaw_temp未被初始化   //需要用串口看的数据
	yaw_temp = angle[0];                     //cml:yaw_temp有了第一个值
	ob=angle[0];
