...
{int speed;
			...
			l=(RC_CtrlData.rc.ch0-1024+RC_CtrlData.mouse.x*9);//右边左右
			f=(RC_CtrlData.rc.ch3-1024);	//左边垂直
			p=(RC_CtrlData.rc.ch2-1024);//左边左右
			g=(RC_CtrlData.rc.ch1-1024-RC_CtrlData.mouse.y*9);//右边垂直
      ...
      SetFrictionWheelSpeed(0);			
		...
		if(RC_CtrlData.mouse.press_r||s2==1) 
		{
			shoot=1000;
			SetSFrictionWheelSpeed(1500);
		}
		else 
		{
			shoot=0;
      SetSFrictionWheelSpeed(0);			
		}
	
//		else if(f!=0||bolun!=0||p!=0)
//		{
				speed=(f*10);
				forward(speed);
		if(s1!=3)
...
//				IMU_G+=((float)(l+roll_angle)/20);
	...
	//		if(Y<200)Y+=360;
		...
		//	sin_yaw = sin(-chassis_move_control->chassis_yaw_motor->relative_angle);
//	cos_yaw = cos(-chassis_move_control->chassis_yaw_motor->relative_angle);
//	chassis_move_control->vx_set = cos_yaw * CM + sin_yaw * vy_set;
...
//		flag_genshui=0;
//	}
//	else if(s1!=3&&flag_genshui==0) {flag_zw=0;Y=REAL_YY;flag_genshui=1;}
}
