...
void IMU_AHRSupdate(void) {
  /*  float norm;
    ...
    float q3q3 = q3*q3;   */
	  ...
	  float halfT;
	/*卡尔曼滤波加入定义*/
	  float K_0,T_0,U_0;//含有卡尔曼增益的另外一个估值，用于计算最优估计值
    float K_1,T_1,U_1;//含有卡尔曼增益的函数，用于计算最优估计值的偏差
    float Y_0,P_0,L_0;
    float Y_1,P_1,L_1;
    float Rate1,Rate2,Rate3;//去除偏差后的角速度
    float Pdot[4],YPdot[4],Rpdot[4];//过程协方程差的微分矩阵
    float angle_err,angle_err2,angle_err3;//角度偏量
    float Pitchcount,Yawcount,Rollcount;//计算过程量
    static float q_bias = 0,q_bias_Y=0,q_bias_R=0;        //陀螺仪的偏差               
    static float P[2][2] = {{1,0},{0,1}},Y[2][2] = {{1,0},{0,1}},R[2][2] = {{1,0},{0,1}};//过程协方差矩阵
/*卡尔曼滤波加入定义如上*/
...
//??????????????  
/*****Pitch,Yaw,Roll轴卡尔曼滤波*******/		
Pdot[0] = Q_angle - P[0][1] - P[1][0];//??????       
Pdot[1] = - P[1][1];                       
Pdot[2] = - P[1][1];                                 
Pdot[3] = Q_gyro;//??????                       
kalmananglepitch+= Rate1 * dt; //角速度积分得出角度
P[0][0] += Pdot[0] * dt; //计算协方差矩阵
P[0][1] += Pdot[1] * dt;
P[1][0] += Pdot[2] * dt;
P[1][1] += Pdot[3] * dt;
angle_err = mygetqval[0] - kalmananglepitch; //计算角度差
Pitchcount = R_angle + P[0][0];
K_0 = P[0][0] / Pitchcount; //计算卡尔曼增益
...
YPdot[2] = - Y[1][1];                                 
YPdot[3] = Q_gyro;//??????                       
kalmanangleyaw+= Rate2 * dt; //角速度积分得出角度
Y[0][0] += YPdot[0] * dt; //计算协方差矩阵
Y[0][1] += YPdot[1] * dt;
Y[1][0] += YPdot[2] * dt;
Y[1][1] += YPdot[3] * dt;
angle_err2 = mygetqval[1] - kalmanangleyaw; //计算角度差
Yawcount = R_angle + Y[0][0];
T_0 = Y[0][0] / Pitchcount; //计算卡尔曼增益
T_1 = Y[1][0] / Pitchcount;
P_0 = Y[0][0]; 
P_1 = Y[0][1];
Y[0][0] -= T_0 * P_0; //跟新协方差矩阵
Y[0][1] -= T_0 * P_1;
Y[1][0] -= T_1 * P_0;
Y[1][1] -= T_1 * P_1;
...
Rpdot[2] = - R[1][1];                                 
Rpdot[3] = Q_gyro;//??????                       
kalmanangleroll+= Rate3 * dt; //角速度积分得出角度
R[0][0] += Rpdot[0] * dt; //计算协方差矩阵
R[0][1] += Rpdot[1] * dt;
R[1][0] += Rpdot[2] * dt;
R[1][1] += Rpdot[3] * dt;
angle_err3 = mygetqval[2] - kalmanangleroll; //计算角度差
Rollcount = R_angle + R[0][0];
U_0 = R[0][0] / Pitchcount; //计算卡尔曼增益
U_1 = R[1][0] / Pitchcount;
L_0 = R[0][0]; 
L_1 = R[0][1];
R[0][0] -= U_0 * L_0; //跟新协方差矩阵
R[0][1] -= U_0 * L_1;
R[1][0] -= U_1 * L_0;
R[1][1] -= U_1 * L_1;
...
{
	MPU6050_Real_Data.Gyro_X = mygetqval[3];	//三轴角速度
	...
	yaw_angle = 1*yaw_temp + yaw_count*360;  //yaw轴角度，被常晓光改成2倍的了
	pitch_angle = angle[1];
    roll_angle = angle[2];	
}
