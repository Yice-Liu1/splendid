...
PID_Regulator_t ShootbolunPID = SHOOT_MOTOR_SPEED_PID_DEFAULT;
PID_Regulator_t CAN2ShootMotorSpeedPID5 = SHOOT_MOTOR_SPEED_PID_DEFAULT;
PID_Regulator_t CAN2ShootMotorSpeedPID6 = SHOOT_MOTOR_SPEED_PID_DEFAULT;
/*--------------------------------------------CTRL Variables----------------------------------------*/
...
void power_limit()                         //功率限制
{
	float total_current;
	if((rest_power-power*0.26f)<7.0f)
	{
		 total_current=abs(CM1SpeedPID.output)+abs(CM2SpeedPID.output)+abs(CM3SpeedPID.output)+abs(CM4SpeedPID.output);
		 CM1SpeedPID.output=CM1SpeedPID.output/(total_current+1)*3000.0f*(0.1f+rest_power*0.15f);
		 CM2SpeedPID.output=CM1SpeedPID.output/(total_current+1)*3000.0f*(0.1f+rest_power*0.15f);
		 CM3SpeedPID.output=CM1SpeedPID.output/(total_current+1)*3000.0f*(0.1f+rest_power*0.15f);
		 CM4SpeedPID.output=CM1SpeedPID.output/(total_current+1)*3000.0f*(0.1f+rest_power*0.15f);
	}
}
void remote(void)
{int speed;
	  ...
	  if(l==-1024) stop();
		else if(f!=0||l!=0||p!=0)
		{
    	 ...
    	 if(G>=257)
	   	 G=257;
     if(G<=227)                                 //右侧上下控制pitch
		  G=227;
		
		/*******s2=1时不拨弹，摩擦轮不转
		        s2=3时，摩擦轮转，挡板后退，拨弹轮转，卡弹后停止
		        s2=2时，摩擦轮转，拨弹轮反转一下下，挡板前进************/
		if(s2==1)
		{ tim=0;                      
 			shan=0;
			chen=0;                     //标志清零
			...
			bospeed=0;
			faspeed=0;
			HNADPIDSPEED(bospeed);                      //s2=1时不拨弹，摩擦轮不转
	...
	stop();
		...
		if(REAL_YY-Y>180){
				Y=Y+360;
		}else if(REAL_YY-Y<-180)	{Y=Y-360;}//110-0  360-295
	//		if(Y<200)Y+=360;
...
常晓光摩擦轮PID控制+拨弹轮
******************/
 void FRICTIONPID(int f,int t)
{
    ShootMotorSpeedPID.ref=t;
    ShootMotorSpeedPID.fdb=M6;
	  ShootbolunPID.ref=f;
    ShootbolunPID.fdb=M5;
	
    PID_Calc(&ShootMotorSpeedPID);
	  PID_Calc(&ShootbolunPID);
	if(ShootMotorSpeedPID.output>=4000)
		...
		if(ShootbolunPID.output>=4000)
		{
			ShootbolunPID.output=4000;	
...
//Set_Current(CAN1, -(int16_t)GMYPositionPID.output, -(int16_t)GMPPositionPID.output,ShootbolunPID.output,ShootMotorSpeedPID.output);
...
void HNADPIDSPEED(int h)
{
  ShootbolunPID.ref=h;
	ShootbolunPID.fdb=M5;
	POSITION_PID_Calc(&ShootbolunPID);
	//Set_Current(CAN1,0,0,ShootbolunPID.output,0);
	delay_ms(1);
}
//**********************************************************
 void FRICTIONPID2(int f,int t)
 {
	 CAN2ShootMotorSpeedPID5.ref=f;
   CAN2ShootMotorSpeedPID5.fdb=M2_5;
   CAN2ShootMotorSpeedPID6.ref=t;
   CAN2ShootMotorSpeedPID6.fdb=M2_6;
  
 PID_Calc(&CAN2ShootMotorSpeedPID5);
 PID_Calc(&CAN2ShootMotorSpeedPID6);
	
 if(CAN2ShootMotorSpeedPID5.output>=4000)
		{
			CAN2ShootMotorSpeedPID5.output=4000;	
		...
		if(CAN2ShootMotorSpeedPID6.output>=4000)
		{
			CAN2ShootMotorSpeedPID6.output=4000;	
		...
		Set_Current2(CAN2, -(int16_t)GMYPositionPID.output, -(int16_t)GMPPositionPID.output,CAN2ShootMotorSpeedPID5.output,CAN2ShootMotorSpeedPID6.output);
  delay_ms(10);
	}
