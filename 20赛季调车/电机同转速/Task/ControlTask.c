...
PID_Regulator_t handspeed1PID = CHASSIS_HAND_SPEED_PID_DEFAULT;
PID_Regulator_t handspeed2PID = CHASSIS_HAND_SPEED_PID_DEFAULT;
...
void ControtLoopTaskInit(void)
{
	...
	handspeed1PID.Reset(&handspeed1PID);
}
...
{ 
	...
	handspeed1PID.ref=h;
	handspeed1PID.fdb=r_a1;
	POSITION_PID_Calc(&handspeed1PID);
			if(handspeed1PID.output>=4000)
			handspeed1PID.output=4000;
		if(handspeed1PID.output<=-4000)
			handspeed1PID.output=-4000;
		
    Set_GEThand(CAN1,handspeed1PID.output,0);
	delay_ms(1);
	
}
void HANDPID2(int h)
{	handspeed2PID.ref=h;
	handspeed2PID.fdb=r_a2;
	POSITION_PID_Calc(&handspeed2PID);
	if(handspeed2PID.output>=4000)
			handspeed2PID.output=4000;
		if(handspeed2PID.output<=-4000)
			handspeed2PID.output=-4000;
		Set_GEThand(CAN1,0,handspeed2PID.output);
	delay_ms(1);
}
