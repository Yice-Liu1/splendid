...
PID_Regulator_t CM1RotatePID = CHASSIS_MOTOR_ROTATE_PID_DEFAULT; //Jiexin Zheng
PID_Regulator_t CM2RotatePID = CHASSIS_MOTOR_ROTATE_PID_DEFAULT;
PID_Regulator_t CM3RotatePID = CHASSIS_MOTOR_ROTATE_PID_DEFAULT;
PID_Regulator_t CM4RotatePID = CHASSIS_MOTOR_ROTATE_PID_DEFAULT;
...
//				nishizhen(speed);
		...
		if(s2 == 2)
		{
			...
			if(flag_rotate> 120)flag_rotate = 2;
		}
		else{
			flag_rotate = 0;
		}
		
		 if((l==0&&f==0&&p==0&&bolun==0) || s2 == 1 )
		 {
			CM1Speed_x=0;CM2Speed_x=0;CM3Speed_x=0;CM4Speed_x=0;
		  CM1Speed_y=0;CM2Speed_y=0;CM3Speed_y=0;CM4Speed_y=0;
			CM1Speed_spin=0;CM2Speed_spin=0;CM3Speed_spin=0;CM4Speed_spin=0;
		 }

		if(flag_follow && s2 != 1 )
		{
//			delta_yaw_angle = yaw_offset;
				delta_yaw_angle =	REAL_YY - Y;	
//			CM1Speed_x=0.2f*delta_yaw_angle*60.0f/(180.0f*2.0f*0.076f);//sin*r,步兵长边0.2m，短边0.165m，麦轮半径0.076m
//			CM2Speed_x=0.2f*delta_yaw_angle*60.0f/(180.0f*2.0f*0.076f);//sin*r
//			CM3Speed_x=-0.165f*delta_yaw_angle*60.0f/(180.0f*2.0f*0.076f);//cos*r
//			CM4Speed_x=-0.165f*delta_yaw_angle*60.0f/(180.0f*2.0f*0.076f);//cos*r
//		  CM1Speed_y=0.165f*delta_yaw_angle*60.0f/(180.0f*2.0f*0.076f);//cos*r
//			CM2Speed_y=0.165f*delta_yaw_angle*60.0f/(180.0f*2.0f*0.076f);//cos*r
//			CM3Speed_y=-0.165f*delta_yaw_angle*60.0f/(180.0f*2.0f*0.076f);//sin*r
//			CM4Speed_y=-0.165f*delta_yaw_angle*60.0f/(180.0f*2.0f*0.076f);//sin*r
				if(delta_yaw_angle>5 || delta_yaw_angle<-5)
//			if(bolun > 200 || bolun < -200)
			{
			//	delta_yaw_angle = (delta_yaw_angle / 180.0f) * PI;
				CM1RotatePID.ref = delta_yaw_angle;
				CM2RotatePID.ref = delta_yaw_angle;
				CM3RotatePID.ref = delta_yaw_angle;
				CM4RotatePID.ref = delta_yaw_angle;
				CM1RotatePID.fdb = 0;
				CM2RotatePID.fdb = 0;
				CM3RotatePID.fdb = 0;
				CM4RotatePID.fdb = 0;
				PID_Calc(&CM1RotatePID);
				PID_Calc(&CM2RotatePID);
				PID_Calc(&CM3RotatePID);
				PID_Calc(&CM4RotatePID);
				CM1Speed_spin = 100*CM1RotatePID.output;
				CM2Speed_spin = 100*CM2RotatePID.output;
				CM3Speed_spin = 100*CM3RotatePID.output;
				CM4Speed_spin = 100*CM4RotatePID.output;
				yaw_offset = 0;
			}else{
				CM1Speed_spin = 0;
				CM2Speed_spin = 0;
		...
		{
			delta_yaw_angle = 20;
//			delta_yaw_angle = (10.0f / 180.0f) * PI;
//			CM1Speed_x=0.2f*180.0f*60.0f/(180.0f*2.0f*0.076f);//sin*r,步兵长边0.2m，短边0.165m，麦轮半径0.076m
//			CM2Speed_x=0.2f*180.0f*60.0f/(180.0f*2.0f*0.076f);//sin*r
//			CM3Speed_x=-0.165f*180.0f*60.0f/(180.0f*2.0f*0.076f);//cos*r
//			CM4Speed_x=-0.165f*180.0f*60.0f/(180.0f*2.0f*0.076f);//cos*r
//		  CM1Speed_y=0.165f*180.0f*60.0f/(180.0f*2.0f*0.076f);//cos*r
//			CM2Speed_y=0.165f*180.0f*60.0f/(180.0f*2.0f*0.076f);//cos*r
//			CM3Speed_y=-0.165f*180.0f*60.0f/(180.0f*2.0f*0.076f);//sin*r
//			CM4Speed_y=-0.165f*180.0f*60.0f/(180.0f*2.0f*0.076f);//sin*r
			yaw_old = angle[0];
			CM1RotatePID.ref = delta_yaw_angle;
			CM2RotatePID.ref = delta_yaw_angle;
			CM3RotatePID.ref = delta_yaw_angle;
			...
			PID_Calc(&CM3RotatePID);
			PID_Calc(&CM4RotatePID);
//			CM1Speed_spin = CM1RotatePID.output;
//			CM2Speed_spin = CM2RotatePID.output;
//			CM3Speed_spin = CM3RotatePID.output;
//			CM4Speed_spin = CM4RotatePID.output;
			CM1Speed_spin = 2000;
			CM2Speed_spin = 2000;
			CM3Speed_spin = 2000;
			CM4Speed_spin = 2000;

//			if(CM1RotatePID.output<0)
//			{
//				CM1Speed_spin = -CM1RotatePID.output;
//			}else{
//				CM1Speed_spin = CM1RotatePID.output;
//			}
//			if(CM2RotatePID.output<0)
//			{
//				CM2Speed_spin = -CM2RotatePID.output;
//			}else{
//				CM2Speed_spin = CM2RotatePID.output;
//			}
//			if(CM3RotatePID.output<0)
//			{
//				CM3Speed_spin = -CM3RotatePID.output;
//			}else{
//				CM3Speed_spin = CM3RotatePID.output;
//			}
//			if(CM4RotatePID.output<0)
//			{
//				CM4Speed_spin = -CM4RotatePID.output;
//			}else{
//				CM4Speed_spin = CM4RotatePID.output;
//			}
//			CM1SpeedPID.output = delta_yaw_angle / 25.0f * 16384.0f / 20.0f;
//			CM2SpeedPID.output = delta_yaw_angle / 25.0f * 16384.0f / 20.0f;
//			CM3SpeedPID.output = delta_yaw_angle / 25.0f * 16384.0f / 20.0f;
//			CM4SpeedPID.output = delta_yaw_angle / 25.0f * 16384.0f / 20.0f;
			Gimbal_Set(G,Y,MPU6050_Real_Data.Gyro_Y,MPU6050_Real_Data.Gyro_Z,REAL_YP,yaw);
//			Set_CM_Speed(CAN1,delta_yaw_angle / 25.0f * 16384.0f / 20.0f,delta_yaw_angle / 25.0f * 16384.0f / 20.0f,delta_yaw_angle / 25.0f * 16384.0f / 20.0f,delta_yaw_angle / 25.0f * 16384.0f / 20.0f);
		}else{
