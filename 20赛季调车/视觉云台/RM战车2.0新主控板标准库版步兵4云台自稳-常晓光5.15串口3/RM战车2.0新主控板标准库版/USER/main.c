...
//PID_Regulator_t GMPPositionPID;125  90 170
float err[3],chazhi;
float manifold_y;
float	manifold_x;
...
{   ;		
 ...
 TIM6_Start();   
	...
	while(1)	//xiaolaji&&benji&&shaji&&
	{
		...
		manifold_y=(((float)y_coordinate-180)/180*aq);   //360
	//manifold_x=(((float)x_coordinate/640)*118+6);    //6-124  118  640 320  
		if(manifold_x<350||manifold_x>290)
		{
			manifold_x=(((float)x_coordinate-320)/320*ssh*xsh); 
		}
		else
		{
	...
	//	if((manifold_x-manifold_x_old)!=0) 
			if((manifold_x-manifold_x_old)!=0) 
			{
				if(x_coordinate>340||x_coordinate<300)  
				{
					XC=XC+manifold_x;
				}
...
//				}
		...
		if(YC>110) YC=110;
		if(YC<60) YC=60;
		Gimbal_Set(YC,XC,MPU6050_Real_Data.Gyro_Y,MPU6050_Real_Data.Gyro_Z,REAL_YP,REAL_YY);
