...
/********************************************************************************
		wjn:给电调板发送指令，0x206ID号为0x1FF，0x20AID号为0x2FF
	 cyq:更改为发送三个电调的指令。
*********************************************************************************/

void Set_Gimbal_Current1(CAN_TypeDef *CANx, int16_t gimbal_yaw_iq, int16_t gimbal_pitch_iq)
{
    CanTxMsg tx_message;    
...
void Set_Gimbal_Current2(CAN_TypeDef *CANx, int16_t gimbal_yaw_iq, int16_t gimbal_pitch_iq)
{
		CanTxMsg tx_message;    
...
{   
    ...
    tx_message.Data[2] = (unsigned char)(gimbal_pitch_iq >> 8);
    tx_message.Data[3] = (unsigned char)gimbal_pitch_iq;
    tx_message.Data[4] = (unsigned char)(friction_iq5 >> 8);
    tx_message.Data[5] = (unsigned char)friction_iq5;
    tx_message.Data[6] = (unsigned char)(friction_iq6 >> 8);
    tx_message.Data[7] = (unsigned char)friction_iq6;
    CAN_Transmit(CANx,&tx_message);
...
cxg云台被动轮一体输出
******************/
void Set_FRICTION_Current(CAN_TypeDef *CANx, int16_t gimbal_yaw_iq, int16_t gimbal_pitch_iq,int16_t friction_iq)
{
    CanTxMsg tx_message;    
    tx_message.StdId = 0x1FF;
    tx_message.IDE = CAN_Id_Standard;
