...
//	Y+=((float)(bolun)/3000);	
	...
	//Set_Gimbal_Current2(CAN1,0,Pitch_Fina_Out);	delay_ms(1);
	
	//FRICTIONPID(bospeed);   //s2=1时不拨弹
...
Set_FRICTION_Current(CAN1,Yaw_Fina_Out,Pitch_Fina_Out,ShootMotorSpeedPID.output);
//Set_Current(CAN1, -(int16_t)GMYPositionPID.output, -(int16_t)GMPPositionPID.output,ShootbolunPID.output);
	//Set_FRICTION_Current(CAN1, ShootMotorSpeedPID.output);
...
//	Set_Current(CAN1,0,0,ShootbolunPID.output);
//	delay_ms(1);
//}
